...
const InnerCollection = React.memo(function Collection({
	...
	function renderSubGroup(
		...
		return (
			<StyledTreeItem
				...
				label={subGroup}
			>
				{groupedSongs[group][subGroup].map((song: ClientSong) => (
					<StyledTreeItemSong
						className="!text-sm"
						key={song.md5}
						nodeId={song.md5}
	...
	function renderGroup(
		...
		const id =
			key !== false
				? (groupedSongs[group] as GroupedSongs)[key][0][groupToId[grouping]]
...
function getIconsByGrouping(groupBy: string): [JSX.Element, JSX.Element] {
	switch (groupBy) {
		case "albumArtist":
			return [
				<GroupIcon key="Group Icon" />,
				<GroupOutlinedIcon key="Group Outlined Icon" />,
			];
		case "albumName":
			return [
				<AlbumIcon key="Album Icon" />,
				<AlbumTwoToneIcon key="Album Two Tone Icon" />,
			];
		default:
			return [
				<ExpandMoreOutlinedIcon key="ExpandedMore Icon" />,
				<ExpandLessOutlinedIcon key="ExpandLessOutlined Icon" />,
			];
	}
}
