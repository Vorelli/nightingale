...
const InnerAudioContextProvider = React.memo(function AudioContextProvider({
	...
	const { currentSong, songs, startingTime, currentSongLoading } = useSelector(
		(s: RootState) => s.songs,
	);
		...
		if (!!audio) {
			...
			try {
				...
				tryingToPlay
					.then(() => {
						audio.muted = false;
						if (audio.src.indexOf("streaming") === -1 && currentSong) {
					...
					.catch((err) =>
						console.log("error when trying to check for permissions", err),
					);
			} catch (err) {
