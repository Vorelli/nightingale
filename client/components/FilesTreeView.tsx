...
const InnerFilesTreeView = React.memo(function FilesTreeView({
	groupBy,
	sortBy,
	songs,
}: Props) {
	const [groupedSongs, setGroupedSongs] = useState(
		{} as GroupedSongs | SuperGroupedSongs,
	);
	...
	useEffect(() => {
		window.addEventListener("mousemove", (ev) => {
			let collectionDiv: HTMLDivElement | null =
				...
				(collectionList as RefObject<HTMLDivElement>).current;
		...
		return (
			...
			>
				<TreeView
					...
					expanded={nodes}
					onNodeToggle={(_ev: any, nodeIds: string[]) => {
	...
	FilesList.whyDidYouRender = true;

	return <FilesList groupedSongs={groupedSongs} icons={icons} />;
});
...
function FilesTreeView() {
	...
	return (
		<InnerFilesTreeView songs={songs.songs} groupBy={groupBy} sortBy={sortBy} />
	);
}
