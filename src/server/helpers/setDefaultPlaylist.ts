...
export function playlistFromQueue (queue: string[], playlistId: number): PlaylistSongs[] {
...
export default async function setDefaultPlaylist (app: Application & WithWebsocketMethod): Promise<void> {
  try {
    ...
    const { db } = app.locals
    const currentDefaultPlaylists = db.select().from(playlists).where(eq(playlists.name, 'Default')).all()
    currentDefaultPlaylists.map(playlist => {
      return db.delete(playlistSongs).where(eq(playlistSongs.playlistId, playlist.id)).run()
    })
    db.delete(playlists).where(eq(playlists.name, 'Default')).run()
    const defaultPlaylist = db.insert(playlists).values([{ name: 'Default' }]).returning().all()
