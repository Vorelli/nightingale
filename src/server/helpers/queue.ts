 ...
 */
export function setQueue (app: Application & WithWebsocketMethod, queue: string[]): void {
 ...
 */
export function generateNextQueue (app: Application & WithWebsocketMethod): void {
 ...
 */
export function generatePreviousQueue (app: Application & WithWebsocketMethod): void {
  if (app.locals.md5s.length > 0) {
    const newQueue = app.locals.queues[app.locals.queueIndex].slice()
    switch (app.locals.shuffleBy) {
      default: {
 ...
 */
export function shuffle<T1> (arr: T1[]): T1[] { return arr.sort(() => 0.5 - Math.random()) }
 ...
 */
export function setTimeAndSend (app: Application & WithWebsocketMethod, time: bigint): void {
