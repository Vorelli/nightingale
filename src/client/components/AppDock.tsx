...
const AppDock = ({ windows, handleClick }: Props) => {
                        ...
                        <img
                            className={
                                windowName !== "resume"
                                    ? "h-8 w-8 m-1"
                                    : "h-10 w-10" +
                                      " object-cover bg-transparent relative"
                            }
    ...
    function calculatePos(i: number, windows: Windows): StyleLeftTop {
        ...
        const diff = middle - i;
        const quantity =
            i < middle ? -diff : Math.abs(diff) + +(length % 2 === 0);
        const angle = 90 + 15 * quantity;
        const x =
            zeroPos.x - len / 2 + radius * Math.cos(angle * (Math.PI / 180));
        const y =
            zeroPos.y - len / 2 + radius * Math.sin(angle * (Math.PI / 180));
        return { left: x, top: y };
    }
