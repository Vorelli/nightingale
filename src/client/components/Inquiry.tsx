  ...
  function handleSubmit(ev: React.FormEvent) {
    if (ev.target instanceof HTMLFormElement) {
      ...
      fetch(URL + "/api/inquiry", {
        ...
        .catch((err) =>
          console.log("error occurred when trying to send message", err)
        )
      ...
      {(!hidden && (
        <div className="overflow-y-auto h-full relative col-span-2 m-2 mr-0">
          {(success && (
            ...
            >
              <h4>
                Please utilize the form below to get in contact with me and I'll
                get back to you ASAP. Thanks!
              </h4>
              <label htmlFor="name">Email/Phone/Contact Method:</label>
              <input
                type="text"
                className={
                  (inProg && "opacity-50 ") ||
              ...
              <button
                disabled={inProg}
                className={
                  (inProg && "opacity-50 ") || "" + "btn btn-secondary btn-lg"
                }
                type="submit"
              >
                Send Inquiry
