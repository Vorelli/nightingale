...
const DesktopWindow = (props: {
    ...
    useEffect(() => {
        setFixedPos((pos: Position | undefined): Position | undefined => {
            ...
            const y =
                pos.y + dims.h < windowHeight - minBound * 2 &&
                pos.y >= minBound
                    ? pos.y
                    : tempY;
                ...
                >
                    <header
                        className={
                            ...
                            "w-full h-12 flex justify-between box-border items-center select-none col-span-2 header hover:cursor-move"
                        }
                    >
                        <div
                            className={
                        ...
                        <HeaderBar
                            ...
                            onShowHideClick={(ev: React.MouseEvent) => {
                                ev.stopPropagation();
                                dispatch(
                                    toggleHidden({ name: props.storeName })
                                );
                            }}
                            onMoveToTopClick={(ev: React.MouseEvent) => {
                                ev.stopPropagation();

                                dispatch(
                                    toggleOnTop({ name: props.storeName })
                                );
                            }}
                        />
                    </header>
