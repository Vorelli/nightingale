...
import {
  type Album,
  type DirectoryAndSubElements,
  type FilePathAndMD5,
  type Song
} from "../types/types.js";
import { getAlbumToInsert, getSongsToInsert } from "./dbHelpers.js";
import { type ICommonTagsResult } from "@vorelli/music-metadata";
const musicMetadata = import("@vorelli/music-metadata");
...
export async function loadSongs(appFromSource: Application): Promise<Album[]> {
  app = appFromSource;
  const musicDir = process.env.MUSIC_DIR as string;
  return await processPaths([path.resolve("../", musicDir)]).then(
    async (md5s) => await processMd5s(app, md5s)
  );
}

async function processPaths(
  pathsInMusic: Dirent[] | string[],
  parentDir = ""
): Promise<Array<{ md5: string; filePath: string }>> {
  ...
  for (let i = 0; i < pathsInMusic.length; i++) {
    ...
    if (typeof filePath !== "string" && filePath.isSymbolicLink()) {
      const res = await fs.readlink(path.join(parentDir, filePath.name));
      filePromises.push(
        ...(await processPaths([res])).map(
          async (file) => await Promise.resolve(file)
        )
      );
    } else if (typeof filePath === "string" || filePath.isDirectory()) {
...
async function processDirectories(
  parentDir: string,
  directories: DirectoryAndSubElements[]
): Promise<FilePathAndMD5[]> {
  ...
  return await Promise.all(pathsProcessing).then((paths) => {
    return paths.flat();
  });
}
  ...
  const albumObj: Album | undefined = rows.reduce(
    (acc: Album | undefined, val: innerJoinReturn) => {
      if (acc === undefined) {
        ...
        const albumArtist =
          findArtistName(rows, val.albums.albumArtistId) ?? "Unknown Artist";
        const albums: Album = {
        ...
        if (!acc.artists.includes(val.artists.name))
          acc.artists.push(val.artists.name);
        if (!acc.genres.includes(val.genres.name))
          acc.genres.push(val.genres.name);
      }
      return acc;
    },
    undefined
    ...
    keyPromises.push(
      new Promise((resolve, reject) => {
        if (["name", "yearReleased", "albumArtist"].includes(key)) {
          ...
          const needToDelete = aList.filter((str) => !bList.includes(str));
          const needToAdd = bList.filter((str) => !aList.includes(str));
          const albumArtistI = needToDelete.reduce(
            (acc, v, i) => (acc === -1 && v === a.albumArtist ? i : acc),
            -1
          );
          ...
          entitiesNeededToBeDeleted
            ...
            .then(async (entities) => {
              ...
              return await Promise.all(
                entities.map(async (entity) => await Promise.resolve(entity.id))
              );
            })
            .then(
              async (ids) =>
                await Promise.all(
                  ids.map((id) =>
                    app.locals.db.delete(table).where(eq(table.id, id)).all()
                  )
                )
            )
            .then(async () => {
              needToDelete.length > 0 && console.log("deleted:", needToDelete);
              return await Promise.all(
            ...
            .then(async (existingArr) => {
              const additions = Promise.all(
                needToAdd
                  .filter((_newName, i) => {
                    console.log("type of albumId:", typeof a.albumId);
                    if (
                    ...
                    console.log("length:", existingArr[i].length);
                    return existingArr[i].length === 0;
                  })
                  .map((newName) => {
                    console.log("inserting into:", key, newName);
                    return app.locals.db
                      .insert(table)
                      .values({ name: newName })
                      .returning()
              ...
              return await Promise.all([additions, Promise.all(addsToDb)]).then(
                ([additions, _adds]) => additions
              );
            })
            .then(async (additions) => {
              ...
              for (let i = 0; i < additions.length; i++) {
                for (let j = 0; j < additions[i].length; j++) {
                  if (key === "artists") {
                    const artistId = (additions[i][j] as NewArtists).id;
                    if (
                    ...
                    const genreId = (additions[i][j] as NewGenres).id;
                    if (
              ...
              if (albumArtistI !== -1) {
                ...
                Promise.all<ReturningArtists>(inserts)
                  .then((ret) => {
                    const newAlbumArtist = ret[replacementI];
                    if (typeof albumId === "string") {
                      ...
                      return app.locals.db
                        .update(albums)
                        .set({
                          albumArtistId: newAlbumArtist.id
                        })
                  ...
                  .catch((err) => {
                    console.log("error occurred when doing stuff:", err);
                  });
              }
              await Promise.all(inserts).then(() => {
                needToAdd.length > 0 && console.log("inserted:", needToAdd);
              });
            })
            .then(() => {
              resolve();
          ...
          const rows = app.locals.db
            ...
            .innerJoin(
              ...
              and(
                ...
                eq(albums.albumArtistId, albumArtists.artistId)
              )
            )
        ...
        insertAllIntoDb(app.locals.db, mergedAlbums)
          .then(() => {
            /*
                ...
                */
            console.table(
              songList.flatMap((album) => album.songs.map((s) => s.md5))
            );
...
async function insertAllIntoDb(
  ...
  const artistLookUps = Array.from(albumList.values()).flatMap((album: Album) =>
    album.artists.map(async (artist: string) =>
      db.select().from(artists).where(eq(artists.name, artist)).all()
    )
  );
  ...
  await Promise.all([
    ...
    function insertIntoTable<T1>(table: AnySQLiteTable, values: any[]): T1[] {
      const results: T1[] = [];
      db.transaction((tx) => {
    ...
    const artistInserts = insertIntoTable<ReturningArtists>(
      artists,
      uniqueArtists
    );
    ...
    const albumsToInsert = Array.from(albumList.values()).map((album: Album) =>
    ...
    const albumToId = insertedAlbums.reduce<Record<string, number>>(
      (acc, cur) => {
        if (cur.name !== null) {
          acc[cur.name] = cur.id;
        }
    ...
    return [
      ...
      insertIntoTable<ReturningAlbumArtists>(
        albumArtists,
        albumArtistsToInsert
      ),
...
async function getSongInfo(
  ...
  return await fileDoesntExist(streamingPath)
    ...
    .then(async () => {
      ...
      const tags = (await musicMetadata)
        .parseFile(filePath)
        .then((tags) => tags.common);

      const imageChecking = fileDoesntExist(getPath(app, md5, "jpg"));

      return await Promise.all([tags, imageChecking, duration]).then(
        async ([tags, doesntExist, duration]: [
          ICommonTagsResult,
          boolean,
          number
        ]) => {
          if (
            ...
            fs.writeFile(streamingPath, newImage).catch((err) => {
              console.log(
  ...
  return {
    ...
    artists:
      tags.artists?.length !== undefined && tags.artists.length > 0
        ...
        : [albumArtist],
    genres:
      tags.genre === undefined || tags.genre.length < 0
        ? ["Genre Missing"]
        : tags.genre,
    songs: [
      {
        md5,
        path: path.relative(
          path.resolve(process.env.MUSIC_DIR as string),
          path.resolve(filePath)
        ),
        duration: Number.isNaN(duration) ? 10000 : duration,
