...
describe("queue", () => {
  let app: Application & WithWebsocketMethod;
  let queue: string[];
  ...
  beforeEach(() => {
    ...
    app = {
      locals: { md5s: queue }
    } as unknown as Application & WithWebsocketMethod;
    setQueue(app, queue);
  });
  ...
  it("should set the queue to what is passed in", () => {
    const arr = ["1", "2", "3"];
    setQueue(app, arr);
    expect(app.locals.queues).toStrictEqual([arr]);
  ...
  it("should generate the previous queue in a decent fashion", () => {
    ...
    randoms.forEach((num, _i) => {
      expect(
        Math.abs(1000 / queue.length - num) <= (0.15 * 1000) / queue.length
      ).toBe(true);
    });
  });
