...
export async function loadSongsAndWait(app: Application): Promise<Album[]> {
  try {
    await fs
      .mkdir(path.join(app.locals.__dirname, "../public", "streaming"))
      .catch((err) => {
        if (!(err.code === "EEXIST" || err.code === "ENOENT")) console.log(err);
      });
    const albums = await loadSongs(app);
    const md5s = albums.flatMap((album: Album) =>
      album.songs.map((s: Song) => s.md5)
    );
    const md5ToSong = albums.reduce((acc: Record<string, Song>, album_1) => {
...
export function waitForSongsToLoad(
  req: Request,
  _res: Response,
  next: NextFunction
): void {
  if (req.app.locals.loadingSongs !== undefined) {
    void req.app.locals.loadingSongs.finally(() => {
      next();
    });
  } else {
    next();
